...
pkgs.writeShellScriptBin "qs-cheatsheets" ''
  ...
  EOF
    ...
    if [[ "$CATEGORY" != "root" && ! -d "$HOME/HyprNix/cheatsheets/$CATEGORY" ]]; then
      echo "Error: Category directory '$CATEGORY' not found in cheatsheets/" >&2
      echo "Available categories:" >&2
      echo "  root" >&2
      ls -1 "$HOME/HyprNix/cheatsheets/" | grep -E '^[a-z]' | head -10 >&2
      exit 1
    fi
    ...
    if [ -d "$HOME/HyprNix/cheatsheets" ]; then
      for category_dir in "$HOME/HyprNix/cheatsheets"/*/; do
  ...
  Window {
    ...
    function loadFileContent(filename) {
      ...
      if (selectedCategory === "root") {
        filePath = "${config.home.homeDirectory}/HyprNix/cheatsheets/" + filename;
      } else {
        filePath = "${config.home.homeDirectory}/HyprNix/cheatsheets/" + selectedCategory + "/" + filename;
      }
