{
  ...
  '';
in
  pkgs.writeShellScriptBin "tcli" ''
    #!${pkgs.bash}/bin/bash
    set -euo pipefail

    # --- Program info ---
    #
    # tcli - NixOS System Management CLI
    # ==================================
    #
    #    Purpose: NixOS system management utility for HyprNix distribution
    ...
    #
    # Variables:
    # PROJECT             - Base directory name (ddubsos/HyprNix)
    ...
    # --- Configuration ---
    PROJECT="HyprNix"   #ddubos or HyprNix
    PROFILE="${profile}"
    ...
    verify_hostname() {
      ...
      if [ -f "$FLAKE_NIX_PATH" ]; then
        ...
        if [ "$current_hostname" != "$flake_hostname" ]; then
          ...
          echo "" >&2
          echo "Hint: Run 'tcli update-host' to automatically update flake.nix" >&2
    ...
    print_help() {
      echo "HyprNix CLI Utility -- version $VERSION"
      echo ""
      echo "Usage: tcli [command] [options]"
      echo ""
      echo "Commands:"
      ...
      echo "                    (Opt: tcli update-host [hostname] [profile])"
      echo ""
    ...
    case "$1" in
      cleanup)
        ...
        LOG_DIR="$HOME/tcli-cleanup-logs"
        mkdir -p "$LOG_DIR"
        LOG_FILE="$LOG_DIR/tcli-cleanup-$(${pkgs.coreutils}/bin/date +%Y-%m-%d_%H-%M-%S).log"
      ...
      update-host)
        ...
        if [ "$#" -eq 3 ]; then # tcli update-host <hostname> <profile>
          target_hostname="$2"
          target_profile="$3"
        elif [ "$#" -eq 1 ]; then # tcli update-host (auto-detect)
          ...
          if [ -z "$target_profile" ]; then
            ...
            echo "Usage: tcli update-host [hostname] [profile]" >&2
            exit 1
          fi
      ...
      doom)
        if [ "$#" -lt 2 ]; then
          ...
          echo "Usage: tcli doom [install|status|remove|update]" >&2
          exit 1
        fi
        ...
        ensure_doom_enabled() {
          ...
          if ${pkgs.gnugrep}/bin/grep -Eq '^[[:space:]]*doomEmacsEnable[[:space:]]*=' "$host_vars_file"; then
            ...
            echo "" >> "$host_vars_file"
            echo "  # Enabled by tcli doom on $(date)" >> "$host_vars_file"
        ...
        case "$doom_subcommand" in
            ...
            if ! is_doom_enabled; then
              ...
              if [[ $REPLY =~ ^[Yy]$ ]]; then
                ...
                if ${pkgs.nh}/bin/nh os switch --diff always --hostname "$PROFILE"; then
                  ...
                  echo "Error: Rebuild failed. Please fix the build and re-run 'tcli doom install'." >&2
                  exit 1
                fi
              else
          ...
          status)
            if [ -x "$HOME/.emacs.d/bin/doom" ] && [ -f "$HOME/.emacs.d/core/doom.el" ]; then
              ...
              echo "Run 'tcli doom install' to install it"
            fi
            ;;
          remove)
            if [ ! -d "$HOME/.emacs.d" ]; then
          ...
          update)
            if [ ! -x "$HOME/.emacs.d/bin/doom" ] || [ ! -f "$HOME/.emacs.d/core/doom.el" ]; then
              echo "Error: Doom Emacs is not installed correctly. Run 'tcli doom install' first." >&2
              exit 1
            fi
