...
print_success_banner() {
  echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║                 HyprNix Installation Successful!                      ║${NC}"
  echo -e "${GREEN}║                                                                       ║${NC}"
...
print_failure_banner() {
  echo -e "${RED}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${RED}║                 HyprNix Installation Failed!                          ║${NC}"
  echo -e "${RED}║                                                                       ║${NC}"
...
print_header "⚠️  CRITICAL WARNING - Existing HyprNix Detected"
...
if [ -d "HyprNix" ]; then
  ...
  echo -e "${RED}║                                                                       ║${NC}"
  echo -e "${RED}║  An existing HyprNix installation was detected at ~/HyprNix           ║${NC}"
  echo -e "${RED}║                                                                       ║${NC}"
  ...
  echo -e "${GREEN}HyprNix exists, backing up to .config/HyprNix-backups folder.${NC}"
  if [ -d ".config/HyprNix-backups" ]; then
    echo -e "${GREEN}Moving current version of HyprNix to backups folder.${NC}"
    mv "$HOME"/HyprNix .config/HyprNix-backups/"$backupname"
    sleep 1
  else
    echo -e "${GREEN}Creating the backups folder & moving HyprNix to it.${NC}"
    mkdir -p .config/HyprNix-backups
    mv "$HOME"/HyprNix .config/HyprNix-backups/"$backupname"
    sleep 1
  fi
else
  echo -e "${GREEN}Thank you for choosing HyprNix.${NC}"
  echo -e "${GREEN}I hope you find your time here enjoyable!${NC}"
fi

print_header "Cloning HyprNix Repository"
git clone https://gitlab.com/zaney/HyprNix.git -b main --depth=1 ~/HyprNix
cd ~/HyprNix || exit 1
